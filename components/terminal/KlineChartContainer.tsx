...
import { getCanonicalSymbol, symbolsMatch } from '@/lib/market-symbols';
...
import {
    ...
    invalidateTerminalChartSnapshot,
    publishTerminalChartReadySnapshot,
...
interface AtomicChartSwitchSeed {
    ...
    bars: OHLCBar[];
    preparedAtMs: number;
    source: 'layout-seed' | 'ready-snapshot';
    hasLivePriceSeed: boolean;
}

const RESOLUTIONS: ResolutionOption[] = DEFAULT_CHART_RESOLUTIONS;

function isChartPerfDebugEnabled(): boolean {
...
function markChartSwitchPerf(label: string, detail?: Record<string, unknown>) {
    if (!isChartPerfDebugEnabled() || typeof window === 'undefined' || !window.performance?.mark) {
        return;
    }
...
function scheduleChartBackgroundTask(callback: () => void, timeout = 250): () => void {
...
const MAX_SHARED_INITIAL_CHART_HISTORY_REQUESTS = 64;
const sharedInitialChartHistoryRequests = new Map<string, Promise<OHLCBar[]>>();
const sharedInitialChartHistoryWaiters = new Map<string, Set<{
    resolve: (bars: OHLCBar[]) => void;
    reject: (error: unknown) => void;
}>>();

function touchSharedInitialChartHistoryRequest(key: string, request: Promise<OHLCBar[]>) {
    sharedInitialChartHistoryRequests.delete(key);
    sharedInitialChartHistoryRequests.set(key, request);
    while (sharedInitialChartHistoryRequests.size > MAX_SHARED_INITIAL_CHART_HISTORY_REQUESTS) {
        const oldestKey = sharedInitialChartHistoryRequests.keys().next().value;
        if (!oldestKey) {
            break;
        }
        sharedInitialChartHistoryRequests.delete(oldestKey);
    }
}

function getOrCreateSharedInitialChartHistoryRequest(
    key: string,
    run: () => Promise<OHLCBar[]>,
) {
    const existing = sharedInitialChartHistoryRequests.get(key);
    if (existing) {
        touchSharedInitialChartHistoryRequest(key, existing);
        return existing;
    }

    const request = run();
    touchSharedInitialChartHistoryRequest(key, request);
    const waiters = sharedInitialChartHistoryWaiters.get(key);
    if (waiters) {
        sharedInitialChartHistoryWaiters.delete(key);
        for (const waiter of waiters) {
            request.then(waiter.resolve, waiter.reject);
        }
    }
    return request;
}

function waitForSharedInitialChartHistoryRequest(key: string) {
    const existing = sharedInitialChartHistoryRequests.get(key);
    if (existing) {
        touchSharedInitialChartHistoryRequest(key, existing);
        return existing;
    ...
    return new Promise<OHLCBar[]>((resolve, reject) => {
        const waiters = sharedInitialChartHistoryWaiters.get(key) ?? new Set();
        waiters.add({ resolve, reject });
        sharedInitialChartHistoryWaiters.set(key, waiters);
    });
}
...
const BACKFILL_CHART_HISTORY_LIMIT = 100;
const BACKFILL_CHART_HISTORY_BASE_DELAY_MS = 300;
const MAX_HISTORY_READINESS_RETRY_ATTEMPTS = 8;
...
function getFreshnessTimestampMsFromLiveSnapshot(snapshot: unknown): number | null {
    ...
    return normalizeTimestampMs(
        ...
        source.server_time_msc ??
        source.serverTimeMsc ??
        source.mt5_time_msc ??
        source.mt5TimeMsc ??
        source.time,
    );
...
function normalizeSymbolForMarketClass(value: string): string {
    return value.trim().replace(/[^A-Za-z0-9]/g, '').toUpperCase();
}

function isCryptoMarketSymbol(symbol: string, category?: string | null): boolean {
    const normalizedCategory = category?.trim().toLowerCase() ?? '';
    if (normalizedCategory.includes('crypto')) {
        return true;
    }

    const normalizedSymbol = normalizeSymbolForMarketClass(symbol);
    return CRYPTO_SYMBOL_PREFIXES.some((prefix) => normalizedSymbol.startsWith(prefix));
}
...
function alignLiveBarOpenToPreviousClose(
    ...
    if (
        !previousBar ||
        (!isOpenLiveTickBar(bar) && bar.closed !== false) ||
        bar.time <= previousBar.time ||
        (bucketMs !== undefined && bucketMs > 0 && bar.time - previousBar.time > bucketMs) ||
...
function alignOpenGapsToPreviousClose(bars: OHLCBar[], bucketMs?: number): OHLCBar[] {
    return bars.map((bar, index) =>
        alignLiveBarOpenToPreviousClose(
            bar,
...
function preferRealBar(
    ...
    if (existingIsReal && incomingIsReal) {
        ...
        if (existingIsLive !== incomingIsLive && existingIsHistorical !== incomingIsHistorical) {
            ...
            // Same-bucket tip: always blend live close into the forming candle.
            // Do not freeze on historical close when wall-clock/closed flags lag.
            const sameBucket =
                historicalBar.time === liveBar.time
                || historicalBar.time === existing.time
                || liveBar.time === existing.time;
            const liveIsOpenTip = liveBar.closed !== true;
            if (sameBucket && liveIsOpenTip) {
                return {
                    ...liveBar,
                    time: existing.time,
                    open: historicalBar.open,
                    high: Math.max(historicalBar.high, liveBar.high, liveBar.close),
                    low: Math.min(historicalBar.low, liveBar.low, liveBar.close),
                    close: liveBar.close,
...
function shouldRetryHistoryBackfill(stats: HistoryBarStats, bucketMs: number): boolean {
    ...
    // Continuity-heavy history is not, by itself, a failed baseline. A response
    // with enough real candles can be painted immediately; any actual seam/range
    // hole is repaired by the targeted gap request after reconciliation.
    return sparseAcrossRange || veryFewRealBars || staleLatestBar;
}
...
function KlineChartContainer({
    ...
    const isAuthenticatedOhlcClientReady = useWebtraderStore((state) => Boolean(
        (
            state.ohlcWsClient?.isConnected &&
            state.ohlcWsClient.isAuthenticated &&
            state.ohlcWsClient.supportsOhlcActions
        ) || (
            state.wsClient?.isConnected &&
            state.wsClient.isAuthenticated &&
            state.wsClient.supportsOhlcActions
        ),
    ));
    ...
    /** Timestamp of last accepted real OHLC live bar apply; used to gate quote→body. */
    const lastOhlcLiveBarAtMsRef = useRef(0);
    ...
    const onHistoryStatusChangeRef = useRef(onHistoryStatusChange);
    ...
    useEffect(() => {
        ...
        onHistoryStatusChangeRef.current = onHistoryStatusChange;
    }, [onUpdatePosition, onClosePosition, onHistoryStatusChange]);
    ...
    const selectedStoreSymbol = useMemo(
        () => storeSymbols.find((entry) => symbolsMatch(entry.name, symbol)),
        [storeSymbols, symbol],
    );
    const selectedSymbolIsCrypto = useMemo(
        () => isCryptoMarketSymbol(
            selectedStoreSymbol?.name ?? symbol,
            selectedStoreSymbol?.category,
        ),
        [selectedStoreSymbol?.category, selectedStoreSymbol?.name, symbol],
    );
    ...
    // Imperative live-price axis sync — avoids React re-renders on every tick.
    // MUST use the same price basis as the forming candle tip (getLiveQuotePrice = mid/last),
    // not bid-only. Bid-only made the dashed line sit away from candle close (e.g. 63899 vs 63904).
    useEffect(() => {
        const syncAxisFromSnapshot = () => {
            let candidate: number | null = null;
            if (typeof preferredLiveAxisPrice === 'number' && preferredLiveAxisPrice > 0) {
                candidate = preferredLiveAxisPrice;
            } else {
                const snap = getLivePriceSnapshotBySymbol(symbol);
                candidate = snap ? getLiveQuotePrice(snap) : null;
            }
            preferredLiveAxisPriceRef.current =
                typeof candidate === 'number' && Number.isFinite(candidate) && candidate > 0
                    ? candidate
                    : null;
            // Prefer live quote for the overlay; fall back to last candle close.
            livePriceRef.current =
                preferredLiveAxisPriceRef.current
                ?? resolveLiveAxisPrice(fallbackLiveAxisPriceRef.current);
            updateLivePriceOverlayRef.current?.();
        };
        syncAxisFromSnapshot();
        return subscribeToLivePriceBySymbol(symbol, syncAxisFromSnapshot);
    }, [symbol, preferredLiveAxisPrice, resolveLiveAxisPrice]);
    ...
    // Authoritative MT5 history still reconciles via the single initial hydration.
    useLayoutEffect(() => {
        ...
        const readySnapshotBars = readySnapshot?.bars.length
            ? normalizeChartBars(
                readySnapshot.bars,
                bucketMs,
            ...
            : [];
        const isReadySnapshotSeed = readySnapshotBars.length > 0;
        const seed = isReadySnapshotSeed
            ? readySnapshotBars
            : resolveChartSwitchSeedBars(
                symbol,
                selectedResolution.minutes,
                normalizedAccountSessionScopeId,
            );
        ...
        if (seed.length > 0) {
            try {
                ...
                atomicChartSwitchSeedRef.current = {
                    ...
                    source: isReadySnapshotSeed ? 'ready-snapshot' : 'layout-seed',
                ...
                // A ready registry snapshot already passed the history/live seam
                // checks on this account session. Ordinary seeds stay provisional.
                setIsLoading(!isReadySnapshotSeed);
                onHistoryStatusChange?.({
                    symbol,
                    timeframeMinutes: selectedResolution.minutes,
                    isLoading: !isReadySnapshotSeed,
                    hasCandles: isReadySnapshotSeed,
                });
        ...
        setIsLoading(true);
        // Critical: never leave the parent gate thinking candles exist after a clear.
        // Stale hasCandles=true was revealing an empty chart grid on first open / switch.
        onHistoryStatusChange?.({
            symbol,
            timeframeMinutes: selectedResolution.minutes,
            isLoading: true,
    ...
    const notifyHistoryStatusChange = useCallback((status: {
        ...
        const historyStatusChange = onHistoryStatusChangeRef.current;
        if (!historyStatusChange) {
            return;
        }
        ...
        historyStatusChange(nextStatus);
    }, [selectedResolution.minutes, symbol]);
    ...
    const clearPositionLines = useCallback(() => {
        ...
        // Also clear persistent lines (and DOM overlays even if the series is gone).
        for (const [, state] of persistentPositionLinesRef.current.entries()) {
            if (mainSeries) {
            ...
            try { state.mainOverlay.remove(); } catch { }
            try { state.slOverlay?.remove(); } catch { }
            try { state.tpOverlay?.remove(); } catch { }
        }
    ...
    const chartInstanceLifecycleKey = useMemo(() => JSON.stringify({
        // A concrete account/session scope is the authoritative identity. The
        // store session id is only a pre-scope placeholder and cannot hydrate.
        sessionIdentity: normalizedAccountSessionScopeId ?? tradingSessionId ?? 'pending-session',
    ...
    useEffect(() => {
        ...
        // Reset OHLC freshness so quote→body is not gated by a previous symbol/timeframe.
        lastOhlcLiveBarAtMsRef.current = 0;
        ...
        const requestChartHistory = (
            ...
            run: () => ohlcService.getHistory(
                ...
                {
                    ...(range ?? {}),
                    priority: isForegroundChartRef.current ? 'foreground' : 'background',
                },
            ),
        });
        const referencedAtomicSwitchSeed = atomicChartSwitchSeedRef.current?.streamKey === chartDataStreamKey
            ? atomicChartSwitchSeedRef.current
            : null;
        // On a completely fresh mount the earlier layout effect runs before chart
        // refs exist, so rebuild the atomic seed directly from the session registry.
        // This is what makes a remounted ready chart visible without another loader.
        const registryReadySnapshot = getTerminalChartReadySnapshot(
            normalizedAccountSessionScopeId,
            symbol,
            selectedResolution.minutes,
        );
        const registryReadyBars = registryReadySnapshot?.bars.length
            ? normalizeChartBars(
                registryReadySnapshot.bars,
                Math.max(1, selectedResolution.minutes) * 60_000,
                LIVE_FIRST_SYMBOL_SWITCH_HISTORY_BARS,
            )
            : [];
        const atomicSwitchSeed: AtomicChartSwitchSeed | null = registryReadyBars.length > 0
            ? {
                streamKey: chartDataStreamKey,
                bars: registryReadyBars,
                preparedAtMs: performance.now(),
                source: 'ready-snapshot',
                hasLivePriceSeed: false,
            }
            : referencedAtomicSwitchSeed;
        const atomicSwitchSeedBars = atomicSwitchSeed?.bars ?? [];
        const hasAtomicSwitchSeed = atomicSwitchSeedBars.length > 0;
        const atomicSwitchSeedIsReady = hasAtomicSwitchSeed
            && atomicSwitchSeed?.source === 'ready-snapshot';
        ...
        // Only a registry snapshot that already passed history/live reconciliation
        // may unlock on remount. Memory and switch seeds are provisional paints.
        if (gracefulLoadingTimerRef.current !== null) {
            clearTimeout(gracefulLoadingTimerRef.current);
            gracefulLoadingTimerRef.current = null;
        }
        if (atomicSwitchSeedIsReady) {
            setIsLoading(false);
        ...
        // Non-crypto weekend = always closed (even if a stale last quote looks "fresh").
        // Weekday: closed if bar is stale/holiday AND no fresh live quote.
        const hasFreshLiveQuoteForRollover = (): boolean => {
            const snapshot = getLivePriceSnapshotBySymbol(symbol);
            const snapshotPrice = snapshot ? getLiveQuotePrice(snapshot) : null;
            if (snapshotPrice === null || !Number.isFinite(snapshotPrice) || snapshotPrice <= 0) {
                return false;
            }

            const freshnessTimeMs = getFreshnessTimestampMsFromLiveSnapshot(snapshot);
            return freshnessTimeMs !== null
                && Date.now() - freshnessTimeMs <= CLOSED_MARKET_LIVE_QUOTE_STALE_MS;
        };
        const shouldFreezeLiveTipForClosedMarket = (latestRealMarketTimeMs?: number): boolean => {
            if (selectedSymbolIsCrypto) return false;
            if (isWeekendUtc()) return true;
            const t = latestRealMarketTimeMs
                ?? (currentBarRef.current ? getBarSourceTimeMs(currentBarRef.current) : 0)
                ?? lastBarTimeRef.current
                ?? 0;
            return t > 0 && isLikelyMarketClosed(t) && !hasFreshLiveQuoteForRollover();
        };
        const shouldPauseFrontendRolloverForClosedMarket = (
            latestRealMarketTimeMs: number,
        ): boolean => shouldFreezeLiveTipForClosedMarket(latestRealMarketTimeMs);
        ...
        const flushLiveSeriesUpdates = () => {
            ...
            const revealReady = hasRevealReadyCandles();
            notifyHistoryStatusChange({
                isLoading: !revealReady,
                hasCandles: revealReady,
            });
        ...
        // Only initial hydration / applyBars(markHistoryLoaded: true) may set isHistoryLoaded.
        let isHistoryLoaded = atomicSwitchSeedIsReady;
        // Do NOT pretend paint happened from atomic seed alone.
        // Maps may still be filled from atomic seed bars below; chartHasSetData /
        // renderedBarCount are set when series actually paints (applyBars / setData).
        let hasRenderedAnyCandles = false;
        ...
        type InitialHistoryHydrationState = 'idle' | 'waiting-client' | 'loading' | 'reconciling' | 'settled';
        let initialHistoryHydrationState: InitialHistoryHydrationState = atomicSwitchSeedIsReady
            ? 'settled'
            : 'idle';
        let initialHistoryHydrationPromise: Promise<void> | null = null;
        ...
        let historyReadinessRetryAttempts = 0;
        ...
        let hasAppliedHistoryBaseline = atomicSwitchSeedIsReady;
        let hasSynchronizedHistoryWithLive = atomicSwitchSeedIsReady;
        const hasRevealReadyCandles = () =>
            hasAppliedHistoryBaseline &&
            hasRenderedHistorySeed &&
            hasSynchronizedHistoryWithLive &&
            chartHasSetData &&
            renderedBarCount > 0;
        ...
        // Secondary panes skip metadata-driven history reloads — primary owns the cascade.
        const unsubscribeHistoryMetadata = isSecondaryPane
            ? () => {}
            : ohlcService.addOhlcHistoryMetadataListener((
                metadataSymbol,
                timeframe,
                metadata,
            ) => {
                ...
                if (metadata.event === 'ohlc_history_refreshed') {
                    ...
                    if (
                        historyMetadataIndicatesDeferredOrSparse(metadata) &&
                        (isHistoryLoaded || initialHistoryHydrationState === 'reconciling')
                    ) {
                        requestLatestHistoryGapFill(
                            isHistoryLoaded ? 'preserve-visible' : 'focus-latest',
                        );
                    }
                }
            });
        ...
        const clearHistoryReadinessRetryTimer = () => {
            ...
            historyReadinessRetryAttempts = 0;
        };

        const scheduleHistoryLoadWhenRealtimeReady = () => {
            // The initial request has one owner. Live/metadata/timer callbacks may
            // wake it while the client attaches, but they never start a second full
            // latest-window request after loading/reconciliation has begun.
            if (
                isSecondaryPane ||
                !isForegroundChartRef.current ||
                !isCurrentChartSwitch() ||
                isHistoryLoaded ||
                initialHistoryHydrationPromise !== null ||
                initialHistoryHydrationState === 'reconciling' ||
                initialHistoryHydrationState === 'settled' ||
            ...
            if (historyReadinessRetryAttempts >= MAX_HISTORY_READINESS_RETRY_ATTEMPTS) {
                initialHistoryHydrationState = 'waiting-client';
                return;
            }
            ...
            const retryWhenReady = () => {
                ...
                if (!isRealtimeOhlcClientReady()) {
                    historyReadinessRetryAttempts += 1;
                    if (historyReadinessRetryAttempts >= MAX_HISTORY_READINESS_RETRY_ATTEMPTS) {
                        historyReadinessRetryRequested = false;
                        initialHistoryHydrationState = 'waiting-client';
                        return;
                    ...
                    const retryDelayMs = Math.min(
                        2_000,
                        TRANSIENT_HISTORY_RETRY_COOLDOWN_MS * 2 ** Math.min(historyReadinessRetryAttempts, 4),
                    );
                    historyReadinessRetryTimer = setTimeout(retryWhenReady, retryDelayMs);
                    return;
                }
        ...
        const scheduleHistoryBackfillRetry = (
            ...
            // Once a synchronized baseline is visible, routine freshness work must
            // stay silent and must never cover the chart with the loader again.
            if (hasRevealReadyCandles()) {
        ...
        function requestLatestHistoryGapFill(layoutMode: DataLayoutMode = 'preserve-or-follow-latest') {
            ...
            const latestKnownHistoryTime = getRealBarTimeWindow(
                loadedHistoryBarsByTime.values(),
                bucketMs,
            ).endTime;
            const latestKnownLiveTime = getRealBarTimeWindow(
                liveBarsByTime.values(),
                bucketMs,
            ).endTime;
            const lastKnownTimestamp = latestKnownHistoryTime
                ?? oldestLiveBarTime
                ?? lastBarTimeRef.current;
            if (!lastKnownTimestamp || lastKnownTimestamp <= 0) {
                return false;
            }

            // This is a range repair, never another full latest-window request.
            // Include one overlap bucket so the history/live seam can be verified
            // and deduplicated by applyBars without creating a visual discontinuity.
            const fromUnixMs = Math.max(1, lastKnownTimestamp - bucketMs);
            const requestSignature = [
                symbol,
                selectedResolution.minutes,
                fromUnixMs,
                latestKnownLiveTime ?? lastBarTimeRef.current,
            ].join(':');
        ...
        const renderProvisionalLivePreview = (emptyMessage?: string, allowLiveOnlyPreview = false) => {
            ...
            // Only report hasCandles when candles are actually visible on the series.
            ...
            const revealReady = hasRevealReadyCandles();
            setIsLoading(!revealReady);
            // Live-only paint stays hidden until history and the live seam agree.
            notifyHistoryStatusChange({
                isLoading: !revealReady,
                hasCandles: revealReady,
            });
            return chartHasSetData && renderedBarCount > 0;
        };
        ...
        const applyLiveBarToRenderedSeries = (
            ...
            // Forming tip: always take live close/high/low. preferRealBar/history merge
            // was freezing the last candle body when a closed/historical tip bar won.
            const isOpenLiveTip =
                liveBar.closed !== true
                && (
                    options.replaceSameBucketRange
                    || isOpenLiveTickBar(liveBar)
                    || isLiveTickBar(liveBar)
                    || (typeof liveBar.source === 'string' && /tick|quote|rust_realtime/i.test(liveBar.source))
                );
            let displayLiveBar: OHLCBar;
            if (
                isOpenLiveTip
                && previousDisplayBar
                && previousDisplayBar.time === liveBar.time
            ) {
                const liveClose = liveBar.close;
                displayLiveBar = {
                    ...previousDisplayBar,
                    ...liveBar,
                    time: previousDisplayBar.time,
                    open: previousDisplayBar.open > 0 ? previousDisplayBar.open : liveBar.open,
                    high: Math.max(previousDisplayBar.high, liveBar.high, liveClose),
                    low: Math.min(previousDisplayBar.low, liveBar.low, liveClose),
                    close: liveClose,
                    volume: Math.max(previousDisplayBar.volume || 0, liveBar.volume || 0),
                    closed: false,
                    source: liveBar.source ?? previousDisplayBar.source ?? 'mt5_tick',
                    sourceTimeMs: liveBar.sourceTimeMs ?? previousDisplayBar.sourceTimeMs,
                    derived: undefined,
                    continuity: undefined,
                    isContinuity: undefined,
                    basis: undefined,
            ...
            } else if (options.replaceSameBucketRange && previousDisplayBar?.time === liveBar.time) {
                displayLiveBar = liveBar;
            } else {
                displayLiveBar = preferRealBar(previousDisplayBar ?? undefined, liveBar, bucketMs);
            ...
            // Keep historical open for continuity, but never let this erase a fresher live close.
            const openAligned = alignLiveBarOpenToPreviousClose(
                displayLiveBar,
                previousRenderedBar,
                bucketMs,
            );
            if (isOpenLiveTip && previousDisplayBar?.time === liveBar.time) {
                displayLiveBar = {
                    ...openAligned,
                    close: liveBar.close,
                    high: Math.max(openAligned.high, liveBar.high, liveBar.close),
                    low: Math.min(openAligned.low, liveBar.low, liveBar.close),
                    closed: false,
                };
            } else {
                displayLiveBar = openAligned;
            }
            ...
            if (repairedDisplayLiveBar.repaired) {
                ...
                // Preserve live close after repair clamps (repair only targets polluted extremes).
                if (isOpenLiveTip) {
                    displayLiveBar = {
                        ...displayLiveBar,
                        close: liveBar.close,
                        high: Math.max(displayLiveBar.high, liveBar.close),
                        low: Math.min(displayLiveBar.low, liveBar.close),
                    };
                }
                liveBarsByTime.set(displayLiveBar.time, displayLiveBar);
            }
            // Always paint when tip close/high/low moved — even if series time is "committed".
            ...
            // Keep overlay price = tip close so the dashed line tracks the last candle.
            preferredLiveAxisPriceRef.current = displayLiveBar.close;
            livePriceRef.current = displayLiveBar.close;
        ...
        const applyLivePriceSnapshotToRenderedCandle = (
            ...
            if (shouldFreezeLiveTipForClosedMarket()) {
                // Keep last history tip; hide dashed live line/badge while closed.
                updateLivePriceOverlay();
                return;
            }
            ...
            // Skip only when the tip candle already reflects this live price (OHLC or a
            // prior quote already applied it). Never block quotes solely because an OHLC
            // bar arrived recently — that froze the forming candle when OHLC lag/stale
            // timestamps skipped intermediate price moves.
            if (
                !options.allowWallClockRollover
                && latestRenderedBar
                && typeof livePrice === 'number'
                && Number.isFinite(livePrice)
                && livePrice > 0
                && Number.isFinite(latestRenderedBar.close)
                && Math.abs(latestRenderedBar.close - livePrice) < Math.max(1e-12, Math.abs(livePrice) * 1e-12)
            ) {
                return;
            }
            ...
            if (eventBucketTime <= 0 && !(previousLastBarTime > 0)) {
                return;
            }

            // Always mutate the *rendered tip bar* (by map identity), not a possibly
            // stale previousLastBarTime that can diverge from the series tip.
            const tipBar =
                currentBarRef.current
                ?? (latestSeriesTime !== null ? displayBarsBySeriesTime.get(latestSeriesTime) : null)
                ?? latestRenderedBar
            ...
            const targetTime = shouldRollToCurrentBucket
                ? currentBucketTime
                : (tipBar && tipBar.time > 0)
                    ? tipBar.time
                    : previousLastBarTime > 0
                        ? previousLastBarTime
                        : eventBucketTime > 0
            ...
            // Fast path: force tip OHLC from the SAME live price as the dashed line.
            if (tipBar && tipBar.time === targetTime && !shouldRollToCurrentBucket) {
                const open = tipBar.open > 0 ? tipBar.open : livePrice;
                const forcedTipBar: OHLCBar = {
                    ...tipBar,
                    symbol,
                    timeframeMinutes: selectedResolution.minutes,
                    time: tipBar.time,
                    open,
                    high: Math.max(tipBar.high || open, open, livePrice),
                    low: Math.min(
                        tipBar.low > 0 ? tipBar.low : open,
                        open,
                        livePrice,
                    ),
                    close: livePrice,
                    // Tick volume accumulates (MT5-style); do not freeze at 1.
                    volume: Math.max(0, (tipBar.volume || 0)) + Math.max(1, volumeIncrement),
                    source: 'mt5_tick',
                    sourceTimeMs: Number.isFinite(snapshotTime) && snapshotTime > 0 ? snapshotTime : nowMs,
                    closed: false,
                    derived: undefined,
                    continuity: undefined,
                    isContinuity: undefined,
                ...
                if (!isBarInsideChartSymbolPriceDomain(symbol, forcedTipBar)) {
                    return;
                }
                const liveBar = cacheLiveBar(forcedTipBar, {
                    replaceExisting: true,
                });
                if (!liveBar) {
                    return;
                }
                const applied = applyLiveBarToRenderedSeries(liveBar, previousLastBarTime, {
                    replaceSameBucketRange: true,
                });
                if (applied) {
                    // Keep dashed price line glued to candle close.
                    fallbackLiveAxisPriceRef.current = livePrice;
                    preferredLiveAxisPriceRef.current = livePrice;
            ...
            const candleBase = tipBar ?? latestRenderedBar;
            if (!candleBase) {
            ...
            // Rollover / new bucket: build from tip close → new open candle.
            const openPrice = candleBase.close > 0 ? candleBase.close : livePrice;
            const liveSnapshotBar: OHLCBar = {
                ...
                continuity: undefined,
                isContinuity: undefined,
                // Keep basis empty so isRealMarketBar accepts the bar for series.update.
                basis: undefined,
                cached: undefined,
            };
        ...
        const forceVisibleCandleRolloverToCurrentBucket = (): boolean => {
            ...
            if (shouldPauseFrontendRolloverForClosedMarket(getBarSourceTimeMs(latestRenderedBar))) {
                return false;
            }
            ...
            // Do not manufacture completed candles across a multi-bucket data gap.
            // History backfill owns those buckets; the UI only creates the current tip.
            const limitedFirstRolloverTime = missingBucketCount > 1
                ? currentBucketTime
                : firstRolloverTime;
            ...
            for (
                ...
                const previousBarIsAdjacent =
                    rolloverTime > previousBar.time &&
                    rolloverTime - previousBar.time <= bucketMs;
                const rolloverOpen = previousBarIsAdjacent ? previousBar.close : livePrice;
                const rolloverBar: OHLCBar = {
                    time: rolloverTime,
                    open: rolloverOpen,
                    high: Math.max(rolloverOpen, livePrice),
                    low: Math.min(rolloverOpen, livePrice),
                    close: livePrice,
                    volume: 1,
                    symbol,
                    timeframeMinutes: selectedResolution.minutes,
                ...
                const liveBar = cacheLiveBar(rolloverBar, { replaceExisting: true });
                if (!liveBar) {
                    continue;
                }
        ...
        const runLiveCandleRollover = (): boolean => {
            ...
            if (
                latestRenderedBar &&
                shouldPauseFrontendRolloverForClosedMarket(getBarSourceTimeMs(latestRenderedBar))
            ) {
                return false;
            }
        ...
        const applyBars = (
            ...
            const latestHistoryBaselineDecision = markHistoryLoaded && includeLiveCache
                ...
                : null;
            const shouldSuppressLiveCacheForPendingLatestHistory =
                latestHistoryBaselineDecision !== null &&
                !latestHistoryBaselineDecision.ready;
            ...
            if (!hasCandles && hasRenderedAnyCandles) {
                // Only claim candles if the series still has painted bars.
                if (chartHasSetData && renderedBarCount > 0) {
                    renderResult.renderedCandles = true;
                    renderResult.historySeedApplied = hasRenderedHistorySeed;
                    const revealReady = hasRevealReadyCandles();
                    if (!deferHistoryStatus) { clearLoadingTimer(); setIsLoading(!revealReady); }
                    if (emptyMessage == null) {
                        setNoDataMessage(null);
                    }
                    emitHistoryStatus({ isLoading: !revealReady, hasCandles: revealReady });
                    return renderResult;
                }
            }
            const compactEntries = buildCompactSeriesEntries(
            ...
            if (latestDisplayBar) {
                ...
                if (markHistoryLoaded && renderResult.historySeedApplied) {
                    hasAppliedHistoryBaseline = true;
                    // Do not hide an already trusted snapshot while an authoritative
                    // refresh repairs its latest seam in the background. Cold charts
                    // still require the current history/live decision to pass.
                    hasSynchronizedHistoryWithLive = hasSynchronizedHistoryWithLive
                        || latestHistoryBaselineDecision?.ready !== false;
                }
                ...
                // Closed non-crypto: never leave a live dashed tip from a prior symbol/session.
                updateLivePriceOverlay();
                setNoDataMessage(null);
            ...
            if (markHistoryLoaded && renderResult.historySeedApplied) {
                isHistoryLoaded = true;
                clearLoadingTimer(); setIsLoading(!hasRevealReadyCandles());
            }
        ...
        async function loadInitialHistory() {
            ...
            const historyLimit = INITIAL_CHART_HISTORY_LIMIT;
            const getLoadHistoryRenderLimit = (_extraBars = 0) => INITIAL_CHART_RENDER_BARS;
            ...
            const applyHistoryResult = (history: OHLCBar[] | null | undefined) => {
                ...
                // Gateway history can legitimately arrive as derived bars without a
                // source tag. Classify the exact bars we will paint; otherwise a
                // healthy 100-bar response looks "mostly continuity" and starts an
                // unnecessary second full latest-window request.
                const historyForPaint = history.map((bar) => (
                    bar.cached === true ? bar : { ...bar, cached: true as const }
                ));
                const historyStats = summarizeHistoryBars(historyForPaint, bucketMs);
                clearHistoryReadinessRetryTimer();
                    // Retries exhausted — fall through to apply history now so bars
                // Always paint the first valid authoritative payload. applyBars merges
                // buffered live candles and owns the continuity/reveal decision.
            ...
            try {
                // The initial critical path owns exactly one authoritative request.
                // Live bars accumulated above are merged atomically by applyBars.
                const sharedInitialHistoryKey = [
                    normalizedAccountSessionScopeId,
                    getCanonicalSymbol(symbol),
                    selectedResolution.minutes,
                    historyLimit,
                ].join(':');
                historyRequest = isSecondaryPane
                    ? waitForSharedInitialChartHistoryRequest(sharedInitialHistoryKey)
                    : getOrCreateSharedInitialChartHistoryRequest(
                        sharedInitialHistoryKey,
                        // The initial owner starts immediately once the concrete
                        // scope exists. ohlcService owns client-attachment waiting;
                        // React readiness polling must not delay this request.
                        () => ohlcService.getHistory(
                            ...
                            historyLimit,
                            {
                                cacheFirst: false,
                                cacheOnly: false,
                                priority: isForegroundChartRef.current
                                    ? 'foreground'
                                    : 'background',
                                expectedSessionId: normalizedAccountSessionScopeId,
                            },
                        ),
                    );
                const history = await withTimeout(
                    historyRequest,
                    CHART_HISTORY_TIMEOUT_MS,
                    'Initial OHLC history load timed out',
                );
                if (!isCurrentChartSwitch()) return;
            ...
            if (isCurrentChartSwitch() && loadSequence === historyLoadSequence) {
                // Keep a live-only tip cached/painted behind the loader. It must not
                // reveal the chart until a history baseline has synchronized with it.
                ...
                const retryScheduled = scheduleHistoryBackfillRetry(stats, hasAppliedHistoryBaseline);
                if (
                    (renderedProvisionalLivePreview || hasRenderedSeededCandles) &&
                    hasRevealReadyCandles()
                ) {
                    setHistoryBackfillMessage(null);
                    setIsLoading(false);
                ...
                if (retryScheduled) {
                    if (hasVisibleCandles) {
                        setIsLoading(true);
                        setNoDataMessage(null);
                        setHistoryBackfillMessage('Synchronizing MT5 history with live data...');
                        notifyHistoryStatusChange({ isLoading: true, hasCandles: false });
                        return;
                    }
                    ...
                    if (earlyNoData) {
                        ...
                        if (backgroundHistoryBackfillAttempts < MAX_BACKGROUND_HISTORY_BACKFILL_ATTEMPTS) {
                            historyBackfillTimer = setTimeout(() => {
                                ...
                                backgroundHistoryBackfillAttempts += 1;
                                requestLatestHistoryGapFill('focus-latest');
                            }, getChartHistoryRetryDelayMs(backgroundHistoryBackfillAttempts));
                        }
                        return;
                    }
        ...
        const unsubscribeBars = ohlcService.subscribe(symbol, selectedResolution.minutes, (_s, _tf, bar) => {
            ...
            // Closed non-crypto: allow completed history bars only — no open live tip mutation.
            if (shouldFreezeLiveTipForClosedMarket(getBarSourceTimeMs(bar)) && bar.closed !== true) {
                return;
            }
            ...
            if (!isHistoryLoaded) {
                const hasHistorySeedForLiveRender =
                    hasRenderedHistorySeed ||
                    hasCachedOrHistoricalSeedBars(loadedHistoryBarsByTime.values());

                // Live ticks are cached immediately, but a cold first-open symbol should
                // keep the loading state until MT5 history/cache supplies the baseline.
                if (hasHistorySeedForLiveRender && applyLiveBarToRenderedSeries(liveBar, previousLastBarTime)) {
                    lastOhlcLiveBarAtMsRef.current = Date.now();
                } else if (!hasHistorySeedForLiveRender) {
                    setHistoryBackfillMessage('Loading MT5 candle history before drawing the chart...');
                }
                scheduleHistoryLoadWhenRealtimeReady();
                return;
            ...
            if (applyLiveBarToRenderedSeries(liveBar, previousLastBarTime)) {
                lastOhlcLiveBarAtMsRef.current = Date.now();
            }
        }, { replay: 'latest' });
        ...
        // Do NOT mark history fully loaded — authoritative MT5 hydration still reconciles below.
        const seedBarsForPaint = hasAtomicSwitchSeed
            ? normalizeChartBars(
                atomicSwitchSeedBars,
                bucketMs,
                atomicSwitchSeedIsReady
                    ? MAX_RENDERED_OHLC_BARS
                    : LIVE_FIRST_SYMBOL_SWITCH_HISTORY_BARS,
            )
        ...
        function hydrateInitialHistory() {
            if (!isActiveRenderedChartSwitch()) {
                return;
            }

            // Do not spend the one initial request during the store-to-session
            // handoff. The identity-keyed effect restarts once the concrete scope
            // is available, and that lifetime owns the authoritative hydration.
            if (!normalizedAccountSessionScopeId) {
                initialHistoryHydrationState = 'waiting-client';
                return;
            }

            // Secondary panes: seed from getSyncBars/switch seed + optional one-shot
            // cache-first getHistory. Skip staged warmup/backfill/gap-fill cascade.
            if (isSecondaryPane) {
                ...
                // Prefer in-memory/switch seed — progressive rAF may already be scheduled.
                if (hasAtomicSwitchSeed || cleanRenderableSyncSeedBars.length > 0) {
                    if (
                        !isHistoryLoaded &&
                        !hasAtomicSwitchSeed &&
                        cleanRenderableSyncSeedBars.length > 0
                    ) {
                        const secondarySeedBars = normalizeChartBars(
                    ...
                    notifyHistoryStatusChange({
                        isLoading: false,
                        // Never claim candles from bare hasAtomicSwitchSeed without paint.
                        hasCandles: renderedBarCount > 0 && chartHasSetData,
                    });
                ...
                // Secondary panes always attach to the primary pane's shared
                // authoritative result, even when they painted an optimistic seed.
                // They never own a network latest-window request.
                void loadInitialHistory().finally(() => {
                    if (isCurrentChartSwitch()) {
                        clearLoadingTimer();
                        const hasVisibleCandles = chartHasSetData && renderedBarCount > 0;
                        isHistoryLoaded = hasVisibleCandles;
                        setIsLoading(!hasVisibleCandles);
                        setHistoryBackfillMessage(
                            hasVisibleCandles
                                ? null
                                : 'Waiting for the primary chart history snapshot...',
                        );
                        notifyHistoryStatusChange({
                            isLoading: !hasVisibleCandles,
            ...
            if (
                initialHistoryHydrationPromise !== null ||
                initialHistoryHydrationState === 'loading' ||
                initialHistoryHydrationState === 'reconciling' ||
                initialHistoryHydrationState === 'settled'
            ) {
                return;
            ...
            const hydration = loadInitialHistory().finally(() => {
                if (initialHistoryHydrationPromise === hydration) {
                    initialHistoryHydrationPromise = null;
                }
                if (isCurrentChartSwitch()) {
                    const revealReady = hasRevealReadyCandles();
                    initialHistoryHydrationState = revealReady
                        ? 'settled'
                        : 'reconciling';
                    if (!revealReady) {
                        requestLatestHistoryGapFill('focus-latest');
            ...
            initialHistoryHydrationPromise = hydration;
        }
        const foregroundLifecycle = {
            ...
            activate: () => {
                ...
                // Symbol switch / keep-alive focus: parent forces hasCandles=false until we reconfirm.
                const hasVisibleBars = chartHasSetData && renderedBarCount > 0;
                if (hasVisibleBars && hasRevealReadyCandles()) {
                ...
                if (initialHistoryHydrationPromise !== null) {
                    return;
                }
                if (!isHistoryLoaded || !hasVisibleBars) {
                    if (
                        initialHistoryHydrationState === 'idle' ||
                        initialHistoryHydrationState === 'waiting-client'
                    ) {
                        hydrateInitialHistory();
                    } else if (initialHistoryHydrationState === 'reconciling') {
                        requestLatestHistoryGapFill('focus-latest');
                    }
                    return;
                }
        ...
        return () => {
            ...
            if (!shouldPreserveChartInstanceForSymbolSwitch) {
                ...
                // Symbol switch with preserved chart: drop previous symbol's position lines
                // (e.g. BTC ~63999 lines stuck on GBPJPY) before the next sync.
                clearPositionLines();
                updateLivePriceOverlayRef.current?.();
            }
        };
        // The live-settings and position effects patch the existing chart through
        // refs. Recreating this hydration effect for those callback identities was
        // issuing another full initial history request during account startup.
    ...
    useEffect(() => {
        ...
        if (isForegroundChart && isAuthenticatedOhlcClientReady) {
            foregroundLifecycle.activate();
            return;
        }

        if (isForegroundChart) {
            // Mount once while disconnected so bounded polling remains a fallback;
            // the selector edge above activates again immediately on authentication.
            foregroundLifecycle.activate();
            return;
        }

        foregroundLifecycle.pause();
    }, [isAuthenticatedOhlcClientReady, isForegroundChart]);
    ...
    // Explicit symbol switch: clear previous symbol's price lines before re-syncing.
    useEffect(() => {
        clearPositionLines();
        syncPositionLines();
    }, [symbol, clearPositionLines, syncPositionLines]);
    ...
    const change = activeBar
        ...
        : null;
    // Snapshot read only (no React subscription) so closed-market notice doesn't re-render per tick.
    const displayQuoteSnapshot = getLivePriceSnapshotBySymbol(symbol);
    const displayQuotePrice = (() => {
        if (typeof preferredLiveAxisPrice === 'number' && preferredLiveAxisPrice > 0) {
            return preferredLiveAxisPrice;
        }
        const bid = displayQuoteSnapshot?.bid ?? 0;
        const ask = displayQuoteSnapshot?.ask ?? 0;
        const candidate = bid > 0 ? bid : ask > 0 ? ask : null;
        return typeof candidate === 'number' && Number.isFinite(candidate) && candidate > 0
            ? candidate
            : null;
    })();
    const livePriceFreshnessMs = getFreshnessTimestampMsFromLiveSnapshot(displayQuoteSnapshot);
    const hasFreshDisplayQuote = displayQuotePrice !== null
        && livePriceFreshnessMs !== null
        && Date.now() - livePriceFreshnessMs <= CLOSED_MARKET_LIVE_QUOTE_STALE_MS;
    // Non-crypto weekend always shows closed (even if a stale quote looks "fresh").
    // Weekday: closed when the bar is stale/holiday and there is no fresh live quote.
    const closedMarketNotice = activeBar &&
        !selectedSymbolIsCrypto &&
        (
            isWeekendUtc()
            || (isLikelyMarketClosed(getBarSourceTimeMs(activeBar)) && !hasFreshDisplayQuote)
        )
        ? 'Market closed — showing last available candle history.'
        : null;
    const statusNotice = closedMarketNotice ?? (!activeBar ? historyBackfillMessage : null);
        ...
        >
                        ...
                        <div className="flex items-center gap-2 text-[13px] font-bold tracking-wide" style={{ color: palette.foreground }}>
                            {showTitle ? (
                                <>
                                    ...
                                    <div className={`ml-2 flex items-center gap-1.5 rounded-none px-2 py-0.5 ${closedMarketNotice ? 'bg-amber-500/10' : 'bg-primary/10'}`}>
                                        <div className={`h-1.5 w-1.5 rounded-none ${closedMarketNotice ? 'bg-amber-400' : 'animate-pulse bg-primary'}`} />
                                        <span className={`text-[10px] font-bold uppercase tracking-wider ${closedMarketNotice ? 'text-amber-300' : 'text-primary'}`}>
                                            {closedMarketNotice ? 'Closed' : 'Live'}
                                        </span>
                                    </div>
                                </>
                            ) : null}
            ...
            <div className="relative flex-1 min-h-0" ref={wrapperRef}>
                ...
                <div
                    ...
                    style={{
                        // Never expose a provisional live-only candle while the
                        // historical baseline and live seam are still reconciling.
                        backgroundColor: palette.background,
                        opacity: isLoading ? 1 : 0,
                        visibility: isLoading ? 'visible' : 'hidden',
                    }}
                >
                    <ChartSyncSpinner size={68} color={palette.muted} />
                </div>
